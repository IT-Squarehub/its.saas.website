<script setup lang="ts">

interface BannerSection {
    features: {
        banner: {
            heading: string
            subtitle: string
        }
    }
}

const query = groq`*[_type == "featuresPage"][0] {
  features {
    banner {
      heading,
      subtitle
    }
  }
}`


// Fetch data from Sanity
const { data: bannerData } = await useSanityQuery<BannerSection>(query)
</script>

<template>
    <div class="relative w-full bg-purple-300 text-white text-center py-32">
        <img src="/assets/shapes.png" alt="Hero Image"
            class="sm:block sm:w-[13rem] lg:w-80 xl:w-auto hidden absolute top-0 left-0 object-cover" />
        <div class="flex flex-col items-center justify-center gap-4 px-[10%] sm:px-[20%] xl:px-[30%]">
            <h1 class="md:text-7xl text-4xl font-bold text-black z-10">{{ bannerData?.features.banner.heading}}</h1>
            <p class="md:text-lg text-md font-Inter font-thin text-black z-10">{{ bannerData?.features.banner.subtitle }}</p>
            <button class="bg-white text-black font-semibold px-4 py-2 rounded mt-4">Get Started</button>
        </div>
        <img src="/assets/shapes.png" alt="Hero Image"
            class="sm:block sm:w-[13rem] lg:w-80 xl:w-auto w-40 hidden absolute bottom-0 right-0 object-cover rotate-180" />
    </div>
</template>
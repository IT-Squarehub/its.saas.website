...
const { data: overviewData, error, pending } = await useSanityQuery<OverviewSection>(query);
  ...
  <section class="w-full bg-[url('/assets/net-background.png')] bg-cover bg-center py-32">
    <div class="mx-auto px-10">
      ...
      <div v-if="error" class="text-center text-red-600">
        <p>Oops! Something went wrong while loading the overview section. Please try again later.</p>
      </div>

      <!-- Loading state -->
      <div v-else-if="pending" class="text-center text-gray-600">
        <p>Loading overview section...</p>
      </div>
      ...
      <div v-else>
        ...
        <!-- Title -->
        <div v-if="overviewData?.overview?.heading" class="flex justify-center mb-6" data-aos="fade-up">
          <div class="rounded-lg px-6 py-3">
            <h2 class="text-6xl text-center font-bold text-purple-700">
        ...
        <!-- Description -->
        <p v-if="overviewData?.overview?.subtitle" class="text-xl text-gray-600 max-w-3xl mx-auto mb-16 text-center"
          data-aos="fade-up">
        ...
        <div class="relative max-w-5xl mx-auto">
          ...
          <div v-if="overviewData?.overview?.image?.asset?.url" class="bg-white rounded-lg shadow-xl p-4"
            data-aos="fade-up">

...
const { data, error, pending } = await useSanityQuery<SolutionsSection>(query);
    ...
    <section class="w-full py-32">
        <div class="max-w-6xl mx-auto px-10">
            ...
            <div v-if="error" class="text-center text-red-600">
                <p>Oops! Something went wrong while loading the solutions section. Please try again later.</p>
            </div>

            <!-- Loading state -->
            <div v-else-if="pending" class="text-center text-gray-600">
                <p>Loading solutions section...</p>
            </div>
            ...
            <div v-else>
                ...
                <!-- Title -->
                <div v-if="data?.solutions?.heading" class="text-center mb-4 flex items-center justify-center"
                    data-aos="fade-up">
                ...
                <!-- Description -->
                <p v-if="data?.solutions?.subtitle" class="text-xl text-gray-600 text-center mb-12 max-w-3xl mx-auto"
                    data-aos="fade-up">
                ...
                <div class="grid grid-col-1 md:flex md:flex-col gap-8">
                    <!-- Feature Tiles -->
                    <div v-if="features.length > 0" class="md:flex col-span-1 md:flex-row gap-4" ref="cardsContainer"
                        data-aos="fade-right">
                        <button v-for="feature in features" :key="feature.id" @click="selectedFeature = feature.id"
                            ...
                            <h3 v-if="feature.title" class="text-xl font-semibold mb-2">{{ feature.title }}</h3>
                            <p v-if="feature.description" class="text-sm" :class="[
                                selectedFeature === feature.id
                                    ? 'text-purple-100'
                                    : 'text-gray-600',
                            ]">
                    ...
                    <!-- Feature Image -->
                    <div v-if="selectedFeatureData?.image?.asset?.url" class="flex h-full w-full" data-aos="fade-left">
